...
if (!class_exists('WS_GOOGLE_WEBMASTER_TOOLS')) {    
	...
	class WS_GOOGLE_WEBMASTER_TOOLS {                                                         
		...
		function __construct() {                
			...
			add_action('wp_head', array(&$this, 'ws_add_webmaster_head'));        
			
			add_action('admin_head', array(&$this, 'ws_add_js_scripts'), 5);         
	...
	function ws_settings() { //register settings
	...
	function ws_add_js_scripts() {
		
		 if(get_option('ws_google_analytics_code')){
		 	
			echo get_option('ws_google_analytics_code');
	...
	function ws_add_webmaster_head(){
		...
		if(get_option('ws_google_webmaster_code')){
			?>
			<meta name="google-site-verification" content="<?php echo get_option('ws_google_webmaster_code'); ?>" />
			<?php
		}
		if(get_option('ws_bring_webmaster_code')){
			?>
			<meta name="google-site-verification" content="<?php echo get_option('ws_bring_webmaster_code'); ?>" />
			<?php
		}
		...
		if(get_option('ws_alexa_varify_code')){
			...
			<meta name="alexaVerifyID" content="<?php echo get_option('ws_alexa_varify_code'); ?>" />
